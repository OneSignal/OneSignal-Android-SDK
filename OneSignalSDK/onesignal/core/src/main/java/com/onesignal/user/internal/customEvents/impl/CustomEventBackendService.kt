...
internal class CustomEventBackendService(
    ...
    override suspend fun sendCustomEvent(
        ...
        customEventMetadata: CustomEventMetadata,
    ): ExecutionResponse {
        val body = JSONObject()
        ...
        payload.put("os_sdk", customEventMetadata.toJSONObject())
        customEvent.properties?.toList()?.asReversed()?.forEach { (key, value) ->
        ...
        val response = _httpClient.post("apps/$appId/integrations/sdk/custom_events", jsonObject)
