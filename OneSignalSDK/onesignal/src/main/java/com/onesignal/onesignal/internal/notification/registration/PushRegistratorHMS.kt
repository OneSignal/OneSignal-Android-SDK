...
internal class PushRegistratorHMS(private val _deviceService: IDeviceService) : IPushRegistrator {
    ...
    private suspend fun getHMSTokenTask(context: Context) : IPushRegistrator.RegisterResult {
        ...
        if (!_deviceService.hasAllHMSLibrariesForPushKit()) {
            return IPushRegistrator.RegisterResult(null, IPushRegistrator.RegisterStatus.PUSH_STATUS_MISSING_HMS_PUSHKIT_LIBRARY)
        }
        ...
        if (!TextUtils.isEmpty(pushToken)) {
            ...
            return IPushRegistrator.RegisterResult(pushToken, IPushRegistrator.RegisterStatus.PUSH_STATUS_SUBSCRIBED)
        } else {
            ...
            return if(pushToken != null) {
                ...
                IPushRegistrator.RegisterResult(pushToken, IPushRegistrator.RegisterStatus.PUSH_STATUS_SUBSCRIBED)
            } else {
                Logging.error("HmsMessageServiceOneSignal.onNewToken timed out.")
                IPushRegistrator.RegisterResult(null, IPushRegistrator.RegisterStatus.PUSH_STATUS_HMS_TOKEN_TIMEOUT)
            }
        }
    }
