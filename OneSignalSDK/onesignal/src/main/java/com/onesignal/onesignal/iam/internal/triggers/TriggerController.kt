...
internal class TriggerController(
    private var dynamicTriggerController: DynamicTriggerController
    ) {
    ...
    fun evaluateMessageTriggers(message: InAppMessage): Boolean {
        ...
        if (message.triggers.size == 0) return true

        // Outer loop represents OR conditions
        for (andConditions in message.triggers) {
            if (evaluateAndTriggers(andConditions)) return true
        }
        return false
    }
    ...
    private fun evaluateAndTriggers(andConditions: List<Trigger>): Boolean {
        for (trigger in andConditions) {
            if (!evaluateTrigger(trigger)) return false
        }
        return true
    }
    ...
    private fun evaluateTrigger(trigger: Trigger): Boolean {
        ...
        if (trigger.kind == Trigger.OSTriggerKind.UNKNOWN) return false
        if (trigger.kind != Trigger.OSTriggerKind.CUSTOM) return dynamicTriggerController.dynamicTriggerShouldFire(
            trigger
        )
        ...
        return if (triggerMatchesFlex(trigger.value, deviceValue, operatorType)) true else false
     ...
     */
    fun addTriggers(newTriggers: Map<String?, Any?>) {
        synchronized(triggers) {
            for (key in newTriggers.keys) {
                val value = newTriggers[key]
                triggers[key!!] = value!!
            }
        }
    }

    fun removeTriggersForKeys(keys: Collection<String?>) {
        synchronized(triggers) { for (key in keys) triggers.remove(key) }
    }

    fun getTriggerValue(key: String?): Any? {
        synchronized(triggers) { return if (triggers.containsKey(key)) triggers[key] else null }
    }
}