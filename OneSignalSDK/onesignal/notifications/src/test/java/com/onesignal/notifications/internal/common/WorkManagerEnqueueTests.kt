...
class WorkManagerEnqueueTests : FunSpec({
    ...
    test("notification generation can be enqueued again after WorkManager rejects it") {
        ...
        shouldThrow<IllegalStateException> {
            manager.beginEnqueueingWork(context, "notification-id", 42, payload, 1L, null, false)
        }

        every {
            workManager.enqueueUniqueWork(any(), any(), any<OneTimeWorkRequest>())
        } returns mockk(relaxed = true)
        manager.beginEnqueueingWork(context, "notification-id", 42, payload, 1L, null, false) shouldBe true
        verify(exactly = 2) {
            workManager.enqueueUniqueWork(any(), any(), any<OneTimeWorkRequest>())
        }
