...
The published modules track their public API in committed dumps under `<module>/api/*.api`, generated by the [binary compatibility validator](https://github.com/Kotlin/binary-compatibility-validator). CI runs `apiCheck` alongside Spotless and Detekt, so any change to the public surface fails the build until the dump is regenerated. The point is that a breaking change has to appear as a reviewable diff rather than be caught by eye.

If you change the public API on purpose, regenerate and commit the dumps:
